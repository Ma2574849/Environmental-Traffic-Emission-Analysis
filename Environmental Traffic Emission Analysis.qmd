...
| R version: 4.5.2
| R packages: shiny, ggplot2, dplyr, plotly, DT, QuantPsyc, leaflet
...
This study developed an interactive Shiny App that integrates real-time traffic flow data with link-level CO emission intensity to explore how different vehicle types shape the formation of urban emission hotspots. Across both peak and off-peak periods, CO emissions showed a highly right-skewed distribution with numerous extreme values, confirming strong spatial and temporal heterogeneity in emission intensity. Statistical tests revealed consistently significant positive correlations between traffic flow and emissions, while the level of the relationships varied by vehicle type. Linear modeling indicated that LDVs constitute the dominant contributor to CO emissions under all time conditions, whereas HDVs, although fewer in number, exhibit high per-vehicle emission contributions. NEVs showed no meaningful association with CO levels, aligning with their near-zero tailpipe emissions. Spatial visualizations further demonstrated that the highest-emission road coincide with high flow LDVs routes or HDVs freight routes and vertify the link between localized traffic patterns and emission hotspots. Overall, by combining statistical analysis with interactive visualization, this app provides a practical tool for users to diagnose urban emission problems. What's more, it can provide evidence for proposing targeted traffic and environmental management strategies.

## AI Disclosure

| (1) Warning: Specifying width/height in layout() is now deprecated. Please specify in ggplotly() or plot_ly().
| Tools: ChatGPT
| Appliction: I asked AI this warning, it advised me to adjust the code accordingly by removing the layout(height = ...) and instead setting the height in ggplotly().
| Verification method: Test the code snippet to ensure the function executes correctly.
| (2) Warning in p \<- p + geom_point(aes(text = paste("Site:", ID, : Ignoring unknown aesthetics: text.
| Tools: ChatGPT
| Appliction: I asked AI this warning, it advised me to move the text aesthetic from the geom⎽point() function to the main aes() function, so that ggplot2 won't generate warnings, while plotly can still recognize and use this text information to create hover tooltips
| Verification method: Test the code snippet to ensure the function executes correctly.
| (3) Warning: Error in select: unused arguments (Statistic, Value)
| Tools: Deepseek
| Appliction: I asked AI this warning, and it advised me to replace all select() calls with subset methods in base R
| Verification method: Test the code snippet to ensure the function executes correctly.
| (4) Optimize the app layout
| Tools: Deepseek
| Application: I asked AI how to display statistical test tables horizontally, it advised me to convert each statistical indicator (each row) from vertical to horizontal arrangement (each column), simplify column names and reduce the number of decimal places in numbers to save space, adjust the data frame structure to have only one row, with each column representing a statistical measure.
| Verification method: Run the Shiny App to see if the layout is clearer.
| (5) Optimize the server structure
| Tools: ChatGPT
| Application: I asked AI if the correlation analysis part has the repeated function with statistical test part, because the the outputs are duplicated. AI told me that they did have duplicated problem and advised me to just hold the statistical test part.
| Verification method: Run the Shiny App to see if the modified code lacks analysis results.
